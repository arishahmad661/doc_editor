...
class DocumentRepository {
  ...
  Future<ErrorModel> createDocument(String token) async {
    ErrorModel error = ErrorModel(
        error: "Some Unexpected Error Occurred", data: null);
  ...
  Future<ErrorModel> getDocumentById(String token, String id) async {
    ...
    try {
      ...
      switch (res.statusCode) {
        case 200:
          error = ErrorModel(error: null, data: DocumentModel.fromJson(res.body));
          print(DocumentModel.fromJson(res.body).content[0]);
          break;
        default:
          throw 'This document doe not exist. Please Create a new one.';
      }
    } catch (e) {
      error = ErrorModel(error: e.toString(), data: null);
    }
